...
@dataclass
class AvaxTrendSignalEngine:
    """
    AVAX trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar el uso del motor genérico BTC para AVAX
    - exigir mejor estructura de tendencia
    - reducir entradas largas débiles y tendencias sucias
    - mantener metadata clara para observabilidad
    """

    adx_min: float = 28.0
    atrp_min: float = 0.0040
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("avax_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("avax_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_avax_trend_setup")
            out[sym] = self._build_flat(sym, meta)
