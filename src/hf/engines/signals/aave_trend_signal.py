...
@dataclass
class AaveTrendSignalEngine:
    """
    AAVE trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar la lógica simple/genérica para AAVE
    - exigir estructura de tendencia más limpia
    - bajar setups débiles en entornos de ruido
    - mantener trazabilidad en signal.meta
    """

    adx_min: float = 30.0
    atrp_min: float = 0.0050
    atrp_max: float = 0.0400

    min_ema_sep_atr: float = 0.20
    min_ema_slope_atr: float = 0.028

    breakout_buffer_atr: float = 0.15
    pullback_max_atr: float = 0.90
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            long_bias = (
                ...
                and float(ret_12h or 0.0) > -0.025
                and float(ret_24h or 0.0) > -0.05
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("aave_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("aave_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_aave_trend_setup")
            out[sym] = self._build_flat(sym, meta)
