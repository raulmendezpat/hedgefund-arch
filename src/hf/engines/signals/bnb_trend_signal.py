...
@dataclass
class BnbTrendSignalEngine:
    """
    BNB trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar la lógica simple/genérica en BNB
    - exigir tendencia más limpia
    - reducir entradas débiles en chops
    - dejar trazabilidad explícita en signal.meta
    """

    adx_min: float = 28.0
    atrp_min: float = 0.0030
    atrp_max: float = 0.0200
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("bnb_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("bnb_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_bnb_trend_setup")
            out[sym] = self._build_flat(sym, meta)
