...
@dataclass
class XrpTrendSignalEngine:
    """
    XRP trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar la lógica simple/genérica en XRP
    - endurecer tendencia y calidad de estructura
    - reducir falsas continuaciones
    - dejar trazabilidad explícita en signal.meta
    """

    adx_min: float = 27.0
    atrp_min: float = 0.0030
    atrp_max: float = 0.0180
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("xrp_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("xrp_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_xrp_trend_setup")
            out[sym] = self._build_flat(sym, meta)
