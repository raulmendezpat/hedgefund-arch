...
@dataclass
class LinkTrendSignalEngine:
    """
    LINK trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar la lógica simple/genérica en LINK
    - endurecer estructura de tendencia
    - evitar entradas débiles en tramos laterales
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("link_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("link_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_link_trend_setup")
            out[sym] = self._build_flat(sym, meta)
