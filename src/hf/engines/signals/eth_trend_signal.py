...
@dataclass
class EthTrendSignalEngine:
    """
    ETH trend engine especializado.

    Timestamp lógico: 2026-03-28
    Objetivo:
    - reemplazar la lógica simple/genérica en ETH
    - exigir mejor estructura de tendencia
    - reducir señales débiles en tramos con poca calidad
    - dejar trazabilidad explícita en signal.meta
    """

    adx_min: float = 24.0
    atrp_min: float = 0.0030
    atrp_max: float = 0.0250

    min_ema_sep_atr: float = 0.16
    min_ema_slope_atr: float = 0.022

    breakout_buffer_atr: float = 0.10
    pullback_max_atr: float = 0.85
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("eth_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("eth_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_eth_trend_setup")
            out[sym] = self._build_flat(sym, meta)
