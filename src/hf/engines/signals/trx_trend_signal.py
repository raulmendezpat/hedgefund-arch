...
@dataclass
class TrxTrendSignalEngine:
    """
    TRX trend engine especializado.

    Diseño:
    - más estricto que btc_trend_signal
    - evita usar el motor genérico de BTC para TRX
    - prioriza calidad de estructura sobre frecuencia
    - pensado para dejar trazabilidad clara en signal.meta

    Notas:
    - Timestamp de creación lógica: 2026-03-28
    - Objetivo inicial: reducir longs débiles y pseudo-tendencias en TRX
    """

    adx_min: float = 32.0
    atrp_min: float = 0.0028
    atrp_max: float = 0.0120

    min_ema_sep_atr: float = 0.22
    ema_slope_lookback: int = 6
    min_ema_slope_atr: float = 0.035

    breakout_lookback: int = 20
    breakout_buffer_atr: float = 0.10

    pullback_max_atr: float = 0.55
    ...
    def generate(self, candles: Dict[str, Candle]) -> Dict[str, Signal]:
        ...
        for sym, c in candles.items():
            ...
            meta = {
                ...
                "ret_1h_lag": float(ret_1h or 0.0),
            ...
            if long_bias and not short_bias:
                ...
                meta["signal_notes"].append("trx_specialized_long")
            ...
            if short_bias and not long_bias:
                ...
                meta["signal_notes"].append("trx_specialized_short")
                out[sym] = Signal(symbol=sym, side="short", strength=float(strength), meta=meta)
                continue

            meta["signal_notes"].append("no_clean_trx_trend_setup")
            out[sym] = self._build_flat(sym, meta)
